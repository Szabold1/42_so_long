...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/01/26 11:23:25 by bszabo            #+#    #+#             */
/*   Updated: 2024/01/26 11:23:28 by bszabo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	load_texture(t_data *data, t_img **img, char *path)
{
...
{
	load_texture(data, &data->player_ptr, PLAYER_PATH);
	load_texture(data, &data->exit_ptr, EXIT_PATH);
	load_texture(data, &data->collect_ptr, COLLECTIBLE_PATH);
	load_texture(data, &data->wall_ptr, WALL_PATH);
	load_texture(data, &data->ground_ptr, GROUND_PATH);
}
