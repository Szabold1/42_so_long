		...
		while (game_d->map[row][col])
		{
			if (row == 0 || col == 0 || col == game_d->cols - 1
				...
				if (game_d->map[row][col] != WALL)
					return (err_msg("map is not surrounded by walls", 0), -1);
			col++;
		}
		if (col != game_d->cols)
			return (err_msg("map is not rectangular", 0), -1);
		row++;
	}
	return (0);
}
...
{
	if (game_d->exit < 1)
		return (err_msg("map does not contain an exit", 0), -1);
	else if (game_d->exit > 1)
		return (err_msg("map contains more than 1 exit", 0), -1);
	else if (game_d->collectible < 1)
		return (err_msg("map does not contain any collectibles", 0), -1);
	else if (game_d->player_start < 1)
		return (err_msg("map does not contain starting position", 0), -1);
	else if (game_d->player_start > 1)
		return (err_msg("map contains more than 1 starting position", 0), -1);
	return (0);
}
	...
	while (game_d->map[row])
	{
		...
		while (game_d->map[row][col])
		{
			if (game_d->map[row][col] != PLAYER
				...
				return (err_msg("map contains invalid characters", 0), -1);
			col++;
		}
		row++;
	}
...
int	check_map(char *map_filename, t_data *data)
{
	...
	if (data->map_fd == -1)
		return (err_msg("open failed", 1), -1);
